...
export function LineSpectrum() {
  ...
  useFrame((state, delta) => {
    ...
    // Step size to map the usable frequencies left-to-right
    ...
    // Update smoothed frequency data (left-to-right)
    for (let i = 0; i < POINTS; i++) {
    ...
    // Connect the points with cylinder segments (left-to-right with alternating spiky peaks)
    for (let i = 0; i < SEGMENT_COUNT; i++) {
      ...
      const mult1 = 1.0 + (spikeMultipliers[i] - 1.0) * spikeIntensity
      const mult2 = 1.0 + (spikeMultipliers[i + 1] - 1.0) * spikeIntensity
      ...
      const y1 = smoothedFreq.current[i] * mult1 * sign1
      const y2 = smoothedFreq.current[i + 1] * mult2 * sign2
      ...
      // Color gradient from left to right: blends from colors[0] to colors[2]
      const t = i / POINTS
